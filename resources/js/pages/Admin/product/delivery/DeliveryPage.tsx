...
import { DeliveryGetData, deliveryListSchema } from './components/schema'
...
export default function DeliveryPage({ index, data }: { index: number; data: any }) {
  const userList = deliveryListSchema.parse(data.delivery)
    ...
    <ItemDataProvider>
      ...
      <Main>
        ...
        <div className='-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-x-12 lg:space-y-0'>
          <DeliveryGetDataTable data={userList} columns={getColumns({index})}/>
        </div>
      </Main>
...
function getColumns({ index }: { index: number }): ColumnDef<DeliveryGetData>[] {
...
interface DataTableProps {
  columns: ColumnDef<DeliveryGetData>[]
  data: DeliveryGetData[]
}

function DeliveryGetDataTable({columns, data}: DataTableProps) {
