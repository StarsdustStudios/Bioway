...
import { RentalGetData, rentalPostSchema } from './schema'
import { router, usePage } from '@inertiajs/react'
import { DropdownMenu, DropdownMenuContent, DropdownMenuLabel, DropdownMenuRadioGroup, DropdownMenuRadioItem, DropdownMenuSeparator, DropdownMenuTrigger } from '@/components/ui/dropdown-menu'
import React from 'react'

const postFormSchema = rentalPostSchema;
const putFormSchema = rentalPostSchema;
...
export function ItemDataActionDialog({
  ...
  const onSubmit = async (data: PutDataForm | PostDataForm) => {
    ...
    formData.append('driver_fee', String(Number(data.driver_fee)));

    if (isEdit && currentRow?.id) {
      formData.append('id', String(currentRow.id));
      formData.append('_method', 'PUT');
      console.log('Submit data:', form.getValues());
      router.post(route('product.rental.update', currentRow.id), formData, {
        onSuccess: () => {
          onOpenChange(false);
          form.reset();
        },
      ...
      router.post(route('product.rental.store'), formData, {
        forceFormData: true,
        onSuccess: () => {
  ...
  React.useEffect(() => {
    if (currentRow) {
      form.reset({
        ...
        driver_fee: currentRow.driver_fee,
      });
    }
  }, [currentRow, form]);
