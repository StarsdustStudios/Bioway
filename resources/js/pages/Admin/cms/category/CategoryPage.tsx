...
import { CategoryGetData, categoryGetSchema } from './components/schema'
...
export default function CategoryPage({ index, data }: { index: number; data: any }) {
  // Parse user list
  const userList = categoryGetSchema.parse(data.category)
    ...
    <ItemDataProvider>
      ...
      <Main>
        ...
        <div className='-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-x-12 lg:space-y-0'>
          <CategoryGetDataTable data={userList} columns={getColumns({index})} type={index}/>
        </div>
      </Main>
...
function getColumns({ index }: { index: number }): ColumnDef<CategoryGetData>[] {
...
interface DataTableProps {
  columns: ColumnDef<CategoryGetData>[]
  data: CategoryGetData[]
  type: number
}

function CategoryGetDataTable({ columns, data, type }: DataTableProps) {
