...
const getCategory = (categoryId : number) => {
  ...
  return categoryList.find((category: CategoryGetData) => category.id === categoryId)?.name;
  }
};

export default function PostPage({ index, data }: { index: number; data: any }) {
  // Parse user list
  const userList = postGetSchema.parse(data.posts)
  categoryList = categoryListSchema.parse(data.categories)
    ...
    <ItemDataProvider>
      ...
      <Main>
        ...
        <div className='-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-x-12 lg:space-y-0'>
          <PostGetDataTable data={userList} columns={getColumns({index})} type={index}/>
        </div>
      </Main>
...
function getColumns({ index }: { index: number }): ColumnDef<PostGetData>[] {
  ...
  const dynamicColumns = cmsData[index].cmsColumns.map((key, colIndex) => ({
    ...
    cell: ({ row }: { row: any }) => {
      ...
      return (
        <div className="w-fit text-nowrap">
          {cmsData[index].cmsColDataset[colIndex] === "hero_image" ? (
            <img
              src={value}
              alt="Logo"
    ...
    ,
    enableSorting: cmsData[index].cmsColDataset[colIndex] === "title" ? true : false,
    enableHiding: false,
  }));
    ...
    <div className='flex items-center justify-between'>
      <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
        <Input
          ...
          onChange={(post) =>
            table.getColumn('title')?.setFilterValue(post.target.value)
          }
